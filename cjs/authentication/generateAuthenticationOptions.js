...
const { isoUint8Array, isoBase64URL, generateChallenge } = require('../helpers/index.js'),
...
async function generateAuthenticationOptions(options) {
    const { allowCredentials, challenge = await generateChallenge(), timeout = 60000, userVerification = 'preferred',
        extensions, rpID, } = options;
    ...
    return {
        ...
        allowCredentials: allowCredentials?.map((cred) => {
            if (!isBase64URL(cred.id))
                throw new Error(`allowCredential id "${cred.id}" 不是有效的 base64url 字符串`);
...
module.exports = { generateAuthenticationOptions };