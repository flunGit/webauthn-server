...
const { TPM_ST, TPM_ALG } = require('./constants.js'), { toDataView } = require('../../../helpers/iso/isoUint8Array.js');
/**
 * 将 TPM 认证凭证中的 certInfo 切割为可读的数据块
 */
function parseCertInfo(certInfo) {
    ...
    const qualifiedSigner = certInfo.slice(pointer, pointer += qualifiedSignerLength),
        extraDataLength = dataView.getUint16(pointer); // 获取 `attsToBeSigned` 的预期哈希值
    pointer += 2;
    ...
    const safe = !!certInfo.slice(pointer, pointer += 1)[0], clockInfo = { clock, resetCount, restartCount, safe },
        // TPM 设备固件版本,被认证的名称
    ...
    const attestedName = certInfo.slice(pointer, pointer += attestedNameLength),
        ...
        qualifiedNameLength = dataView.getUint16(pointer); // 被认证的限定名称,可以忽略
    pointer += 2;
