...
const { COSEKEYS, COSECRV, isCOSEAlg } = require('../../cose.js'), { fromBuffer } = require('../isoBase64URL.js'),
...
async function verifyOKP(opts) {
    ...
    const keyData = { kty: 'OKP', crv: _crv, alg: 'EdDSA', x: fromBuffer(x), ext: false },
        keyAlgorithm = { name: _crv, namedCurve: _crv },
        key = await importKey({ keyData, algorithm: keyAlgorithm }), verifyAlgorithm = { name: _crv };

    return WebCrypto.subtle.verify(verifyAlgorithm, key, signature, data);
}

module.exports = { verifyOKP };