...
const { COSEKEYS, isCOSEAlg } = require('../../cose.js'), { fromBuffer } = require('../isoBase64URL.js'),
    { getWebCrypto } = require('./getWebCrypto.js'), { importKey } = require('./importKey.js'),
    { mapCoseAlgToWebCryptoAlg } = require('./mapCoseAlgToWebCryptoAlg.js'),
    { mapCoseAlgToWebCryptoKeyAlgName } = require('./mapCoseAlgToWebCryptoKeyAlgName.js');
...
async function verifyRSA(opts) {
    const { cosePublicKey, signature, data, shaHashOverride } = opts,
        WebCrypto = await getWebCrypto(), alg = cosePublicKey.get(COSEKEYS.alg),
        n = cosePublicKey.get(COSEKEYS.n), e = cosePublicKey.get(COSEKEYS.e);
    ...
    const keyData = { kty: 'RSA', alg: '', n: fromBuffer(n), e: fromBuffer(e), ext: false },
