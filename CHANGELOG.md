# 变更日志
## [2.0.0] - 2026-04-22 08:35
### 重大优化:
- 本包现已丢弃CJS模式编写,改为 ESM 模块编写->未来趋势;只要你的 Node.js 版本大于22.12,可保留CJS `require()` 语法调用,否则请使用 `import` 语法;
- 直接导出isoBase64URL, isoCBOR, isoCrypto, isoUint8Array命名空间函数(命名空间不再支持);以下函数名也优化更名:
...
>> 4. fromASCIIString->asciiToBytes	(将 ASCII 字符串转换为 Uint8Array);
